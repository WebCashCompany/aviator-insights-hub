...
function QuickPrompt({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="text-xs px-3 py-1.5 rounded-full border border-border hover:bg-muted/60 text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1"
    >
...
export default function AIPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setAnalyzeErr('Falha na conexão com a API do Gemini. Verifique sua chave.')
    } finally {
      setAnalyzing(false)
    }
  }
...
<p className="text-xs text-muted-foreground mt-0.5">Gemini 2.5 Flash · {candles.length} velas</p>          </div>
        </div>
        ...
        <div className="glass-card p-5 flex flex-col items-center justify-center text-center space-y-3 border border-primary/20 bg-primary/5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Analisa as últimas {Math.min(candles.length, 60)} velas com Gemini AI
            </p>
          </div>
          <Button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-black/5">
          ...
          {isTyping && (
            <div className="flex items-center gap-2 text-xs text-primary">
              ...
              <span>Gemini está analisando o gráfico...</span>
            </div>
          )}
